...
class NeuralNetwork(NNDirectedWeightedGraph):
    def __init__(self, weights, data_type='continuous', functions=None, derivatives=None, data_points=list(), alpha=0.001, debug=False):
        ...
        if functions is not None and len(functions) == len(self.nodes):
            ...
            self.derivatives = [
                lambda x: 1 for _ in range(0, len(node_indices))]
    ...
    def set_gradients(self):
        ...
        self.intermediate_neuron_gradients = {
            edge: None for edge in self.weights.keys()}

    def set_misclassifications(self):
        self.misclassifications = {
            tuple(data_point['input']): False for data_point in self.data_points}

    def set_predictions(self):
        self.predictions = {
            tuple(data_point['input']): 0.0 for data_point in self.data_points}
    ...
    def update_neuron_gradients(self, data_point, node_index):
        ...
        if self.data_type == 'continuous':
            ...
            prediction = data_point['output'](
                self.predictions[tuple(data_point['input'])])

        self.neuron_gradients[node_index] += 2.0 * prediction
        total_weight = 0.0
        every_possible_path_containing_edge = self.get_every_possible_path_containing_edge(
            current_paths=[list([node_index])])
        ...
        for path in every_possible_path_containing_edge:
            ...
            for i in range(0, len(path) - 1):
                ...
                if self.intermediate_neuron_gradients[edge] is None:
                    ...
                    self.intermediate_neuron_gradients[edge] = self.weights[edge] * \
                        self.derivatives[path[i]](
                            self.nodes[path[i + 1]].input)
    ...
    def mitosis(self):
        ...
        new_alpha = self.alpha * \
            math.e ** (np.random.normal(0, 1) /
                       (2 ** 0.5 * len(new_weights) ** 0.25))
    ...
    def get_node_input(self, node_index):
        ...
        for parent in self.nodes[node_index].parents:
            ...
            result += self.weights[(parent, node_index)] * \
    ...
    def print_outputs(self, iteration):
        ...
        print('\tMisclassifications: {}'.format(
            list(self.misclassifications.values()).count(True)))
    ...
    def print_debugging_variables(self, data_point, edge, dE):
        ...
        print('\tdE', dE)
        print('\tmisclassifications', sum(
            [1.0 for classification in self.misclassifications.values() if classification]))
