...
class NeuralNetwork(NNDirectedWeightedGraph):
    ...
    def set_gradients(self):
        ...
        self.intermediate_neuron_gradients = {edge: 0.0 for edge in self.weights.keys()}
    ...
    def update_neuron_gradients(self, data_point, node_index):
        ...
        for path in every_possible_path_containing_edge:
            ...
            for i in range(1, len(path) - 1):
                ...
                print("self.get_node_input(path[i])",self.get_node_input(path[i]))
                temp *= self.weights[edge] * self.activation_function_derivatives[path[i]](self.get_node_input(path[i]))
                self.intermediate_neuron_gradients[edge] += self.weights[edge] * self.activation_function_derivatives[path[i]](self.get_node_input(path[i]))
            total_weight += temp
