...
all_possible_board_states = open(
    'metaheuristic_algorithm/ttt_board_states/board_states.txt', 'r').readlines()
...
def fight(strategy_one, strategy_two, count_score=True):
    def lambda_strategy_one(
        board_state): return strategy_one['strategy'][board_state]
    def lambda_strategy_two(
        board_state): return strategy_two['strategy'][board_state]
...
original_population = [{key: value for key,
                        value in strategy.items()} for strategy in population]
previous_population = [{key: value for key,
                        value in strategy.items()} for strategy in population]
...
for generation in range(1, num_generations + 1):
    ...
    if generation < 6 or generation % 10 == 0:
    ...
    for strategy in previous_population:
        strategy['score'] = 0
    for strategy in best_five:
        strategy['score'] = 0
    ...
    for strategy in original_population:
        strategy['score'] = 0
    for strategy in best_five:
        strategy['score'] = 0
    ...
    for i, strategy_one in enumerate(best_five):
        ...
        for j, strategy_two in enumerate(best_five[i + 1:]):
            ...
            baby_strategy_one, baby_strategy_two = breed(
