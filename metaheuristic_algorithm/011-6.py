...
for generation in range(num_generations):
    ...
    for i, MHA in enumerate(metaheuristic_algorithms):
        ...
        MHA.determine_fitness(fitness_score='bracket',
                              cutoff_type='stochastic')
        MHA.breed(mutation_rate=0.01)

        original_average_score[i].append(calculate_score(
            MHA.fittest_chromosomes, MHA.original_population))
        previous_average_score[i].append(calculate_score(
        ...
        loss_prevention_score[i].append(
            will_block_score / (can_block_score + 1))
...
plt.legend(['64-0.01', '64-0.001', '256-0.01',
           '256-0.001', '1024-0.01', '1024-0.001'])
