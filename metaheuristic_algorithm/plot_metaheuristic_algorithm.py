...
def plot_win_loss_tie(MHA, plotted_generation_num, random_average_score, original_average_score, previous_average_score):
    ...
    return_dict = {
        # d for 'double' (double precision float)
        'random': multiprocessing.Manager().Value('d', 0),
    ...
    for i, (name, (_, function, enemys)) in enumerate(random_original_previous_iter.items()):  # cringe
        ...
        for j in range(0, available_thread_count):
            ...
            args = [
                ...
                return_dict[name],
                lock,
                int(time.time() + 100*j*math.sin(i) + 350*i*math.cos(j*j) +
                    100*j + 4500*i)  # random expression cause random is cringe
            ]
...
def plot_state_value(MHA, plotted_generation_num, win_prediction_score, lose_prediction_score, tie_prediction_score):
    ...
    win_predictions = int()
    num_win_predictions = len(
        board_states['winning']) * len(MHA.fittest_chromosomes)

    lose_predictions = int()
    num_lose_predictions = len(
        board_states['losing']) * len(MHA.fittest_chromosomes)

    tie_predictions = int()
    num_tie_predictions = len(
        board_states['losing']) * len(MHA.fittest_chromosomes)
...
def plot_strategy_effectiveness(MHA, plotted_generation_num, win_capture_score, loss_prevention_score):
    ...
    winnable_board_states = [line.strip('\n') for line in open(
        'metaheuristic_algorithm/ttt_board_states/winnable_board_states.txt', 'r').readlines()]
    losable_board_states = [line.strip('\n') for line in open(
        'metaheuristic_algorithm/ttt_board_states/losable_board_states.txt', 'r').readlines()]
