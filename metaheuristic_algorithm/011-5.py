...
def calculate_score(fittest_chromosomes, population):
    ...
    for chromosome_one in fittest_chromosomes:
        ...
        for chromosome_two in population:
            ...
            def lambda_chromosome_one(
                board_state, _): return chromosome_one['genes'][board_state]

            def lambda_chromosome_two(
                board_state, _): return chromosome_two['genes'][board_state]
...
ttc_chromosome_genes = [line.strip('\n') for line in open(
    'metaheuristic_algorithm/ttt_board_states/ttc_chromosome_genes.txt', 'r').readlines()]
# ^^^^^ tic tac toe chromosome genes

MHA = MetaHeuristicAlgorithm()
MHA.read_chromosomes(ttc_chromosome_genes, population_size=64)
...
for generation in range(num_generations):
    ...
    MHA.determine_fitness(fitness_score='round robin',
                          cutoff_type='tournament')
    MHA.breed(mutation_rate=0.01)

    original_average_score.append(calculate_score(
        MHA.fittest_chromosomes, MHA.original_population))
    previous_average_score.append(calculate_score(
        MHA.fittest_chromosomes, MHA.previous_population))
        ...
        for board_state in chromosome['genes'].keys():
            ...
            if win_capture[0]:
                ...
                if chromosome['genes'][board_state] in win_capture[1]:
                    will_win_score += 1
            loss_prevention = plots_3_and_4(
                list_board_state, 2)  # loss prevention
            if loss_prevention[0]:
                can_block_score += 1
                if chromosome['genes'][board_state] in loss_prevention[1]:
                    will_block_score += 1

    win_capture_score.append(will_win_score / (can_win_score + 1))
    loss_prevention_score.append(will_block_score / (can_block_score + 1))
